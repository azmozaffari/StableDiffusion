...
The gole of this project is to generate ads by given few number of images of a specific brand. In this project, instead of describing the ad scene in text (prompt), I use a sample image for simplisity. Sometimes we are not caple of defining every details of the scene that we create in our ming. The model uses the sample given image and recreate the scene with the a brand item that we considered.

I have used [DreamBooth](https://github.com/google/dreambooth) to train a StableDiffusion model with few number of sample item. Here my samples item were apple watch and DARA doll. 
